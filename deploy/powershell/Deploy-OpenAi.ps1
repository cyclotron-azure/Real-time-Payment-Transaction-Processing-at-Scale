...
if (-Not ($deployments -Contains $completionsDeployment)) {
    ...
    az cognitiveservices account deployment create -g $resourceGroup -n $name --deployment-name $completionsDeployment --model-name 'gpt-4o-mini' --model-version '2024-07-18' --model-format OpenAI --sku Standard --sku-capacity 120
}
